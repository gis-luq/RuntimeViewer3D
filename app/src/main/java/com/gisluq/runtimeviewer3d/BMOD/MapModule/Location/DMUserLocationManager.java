package com.gisluq.runtimeviewer3d.BMOD.MapModule.Location;
...
import com.gisluq.runtimeviewer3d.Utils.DialogUtils;
...
//    public Location locTemp = null;//位置零时存储
    ...
    public DMUserLocationManager(Context context, SceneView sceneView) {
     ...
     */
    public void start() {
        this.sceneView.getGraphicsOverlays().add(locgraphicOverlay);
    ...
    public void stop() {
        try {
            ...
            this.sceneView.getGraphicsOverlays().remove(locgraphicOverlay);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
//                        Point pt_location = new Point(lon, lat);//GPS设备获取的坐标
                        ...
                        Point pt_mapsr = (Point) GeometryEngine.project(pt_location, sceneView.getSpatialReference());//转换到当前地图坐标
                        if (locgraphicOverlay != null) {
                            ...
                            if (isFirstStart) {
                                Viewpoint viewpoint = new Viewpoint(pt_mapsr,10000);
                                sceneView.setViewpoint(viewpoint);//定位到中心
